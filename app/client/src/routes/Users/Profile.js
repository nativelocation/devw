...
class Profile extends Component {
	...
	onChangePassword(e) {
		...
		if (password !== "") {
			...
			if (password !== re_password) return this.setState({ errorMessage: "Passwords do not match." });
		}
	...
	editUser() {
		const { account } = this.props;
		const { firstName, lastName, email, rol } = this.state;
				...
				<Wrapper title='Profile' onClick={this.onLogout}>
					<div className='mt-3 row'>
						<form onSubmit={this.onSubmit.bind(this)} className='col-md-6'>
							...
							<Button text='Save Changes' filter classes='float-right' />
							<button
								type='button'
						...
						<form onSubmit={this.onChangePassword.bind(this)} className='col-md-6'>
							...
							<Button text='Change Password' filter classes='float-right' />
						</form>
