...
class DailyGoals extends Component {
	...
	onSubmit(e) {
		...
		const data = { userId: account._id, task, taskDate: taskDateFormat, orderList: myGoals.length, priority: "" };
			...
			.then(res => {
				if (res.status === 201) {
					this.setState({
						...
						alertShow: false,
						task: "",
						selectValue: this.state.taskDate,
	...
	showDeleteUserAlert(id, orderList) {
		const alertProps = this.getDeleteAlertProps(id, orderList);
		this.setState({ alertShow: true, alertProps });
	}

	deleteUser(_id, orderList) {
		const { account } = this.props;
		const closeProcess = errorMessage => this.setState({ alertShow: false, errorMessage });
		const myGoals = this.state.tasks.data.filter(t => t.userId === account._id);
		let belowData = myGoals.slice(orderList + 1, myGoals.length);
		let orderListData = [];
		if (belowData.length !== 0) {
			belowData.map(e => {
				orderListData = [...orderListData, { _id: e._id, orderList: e.orderList - 1 }];
			});
		}

		let formData = { _id, orderListData };
		if (_id) {
			Api.LogicDeleteGoal(account.tokenAuth, formData)
				.then(res => {
					if (res.status === 201) {
		...
		if (Object.keys(users).length > 0) {
			...
			const myComponent = (
				<UserGoals
					...
					onUpdateGoals={this.updateGoals.bind(this)}
			...
			const usersGoals = users.data.map((x, i) => {
				...
				if (userGoals.length > 0 && x._id !== account._id)
					return <UserGoals key={i} data={userGoals} user={x} />;
			});

			listUsers = [myComponent, ...usersGoals];
		}
		let custom = showCustom ? (
			<DatePicker
				style={styles.datepicker}
				selected={customDate}
				onChange={this.fromChange.bind(this)}
				customInput={<input style={{ width: "100%", textAlign: "center", fontSize: "14px" }} />}
			/>
		) : (
			undefined
		);
		let selectDateSidebar = (
			<FilterGoals
				selectValue={selectValue}
				customValue={customDateValue}
				onCustomChange={e => this.setState({ customDateValue: e })}
				onSelectChange={e => this.setState({ selectValue: e })}
				onSubmit={e => this.updateGoals(e)}
			/>
		);
		return (
			<Fragment>
				...
				<Wrapper maxWidth={wrapperWidth} title='Daily Goals' hideLink>
					<div>
						<div className='goalsBox mb-3'>{listUsers}</div>
						<form className='formTask' onSubmit={e => this.onSubmit(e)}>
							...
							<div className='mt-2 d-md-flex flex-md-row justify-content-md-end'>
								<div className='col-md-12 d-flex flex-row justify-content-end p-0'>
									<div className='row d-md-flex flex-md-row col-md-12 p-0'>
										<div className='col-md-6 col-sm-3 text-right'>
											<Error text={errorMessage} />
										</div>
										<div className='d-md-flex flex-md-row col-md-2 col-sm-3'>{custom}</div>
										<div className='d-md-flex flex-md-row col-md-2 col-sm-3'>
											<SelectInputGoals
												value={taskDate}
												onChange={this.onChange.bind(this)}
												name='taskDate'
										...
										<div className='d-md-flex flex-md-row col-md-2 col-sm-3'>
											<Button text='Add Goal' classes='float-right' filled filter />
										</div>
									</div>
								</div>
							</div>
	...
	getDeleteAlertProps(id, orderList) {
		return {
			title: "Delete Goal",
			...
			onConfirm: this.deleteUser.bind(this, id, orderList),
			onCancel: () => this.setState({ alertShow: false })
		};
	}
}
